...
const calcDisplayBalance = function (acc) {
  acc.balance = acc.movements.reduce((acc, mov, i) => {
    return acc + mov;
  }, 0);
...
const checkDogs = function (dogsJulia, dogsKate) {
  // const juliaCopy = new Array();
  // dogsJulia.forEach(function (mov) {
  //   juliaCopy.push(mov);
  // });
  ...
  const withNew = [...juliaCopy, ...dogsKate];

  withNew.forEach(function (mov, i) {
    if (mov >= 3) {
...
// Strings  sort默认的排序方式是基于字符的排序，它会转换成字符串然后排序
const owners = ['Jonas', 'Zach', 'Adam', 'Martha'];
console.log(owners.sort());
console.log(owners); //改变了原来的数组
